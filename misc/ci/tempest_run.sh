...
if [ $tempest-full == true ]; then
    ...
    test_cases="tempest.tests.compute.images.test_list_images:ListImagesTest.test_list_images_with_detail
                ...
                tempest.tests.compute.volumes.test_volumes_get:VolumesGetTestJSON.test_volume_create_get_delete
                tempest.tests.identity.admin.test_users:UsersTestJSON.test_get_users
                tempest.tests.volume.test_volumes_get:VolumesGetTest.test_volume_create_get_delete"
    for i in $test_cases
    do
        nosetests -sv --attr=type=smoke $i
    done
fi
