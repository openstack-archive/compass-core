...
def setupDb():
    ...
    with database.session() as session:
            ...
            for name, networking_config in zip(cluster_names,
                                               clusters_networking_config):
                nconfig = deepcopy(CLUSTER_NETWORKING_CONFIG)
                util.merge_dict(nconfig, networking_config)
                c = Cluster(name=name, adapter_id=1,
                            security_config=json.dumps(CLUSTER_SECURITY_CONFIG['security']),
                            networking_config=json.dumps(nconfig['networking']),
                            partition_config=json.dumps(CLUSTER_PARTITION_CONFIG['partition']))
                session.add(c)
            ...
            for mip, tip in zip(host_mips, host_tips):
                config = deepcopy(CLUSTERHOST_CONFIG)
