...
@log_user_action
@login_required
def show_switch_machine(switch_machine_id):
    """get switch machine."""
    data = _get_request_args()
    return utils.make_json_response(
        200,
...
@log_user_action
@login_required
def update_switch_machine(switch_machine_id):
...
@log_user_action
@login_required
def patch_switch_machine(switch_machine_id):
...
@log_user_action
@login_required
def delete_switch_machine(switch_machine_id):
...
def update_clusterhost_config(clusterhost_id):
    ...
    return utils.make_json_response(
        200,
        clusterhost_api.update_clusterhost_config(
            current_user, clusterhost_id, **data
        )
    )
...
def patch_clusterhost_config(clusterhost_id):
    ...
    return utils.make_json_response(
        200,
        clusterhost_api.patch_clusterhost_config(
            current_user, clusterhost_id, **data
        )
    )
