...
def deploy(cluster_id, hosts_id_list, username=None):
    ...
    with util.lock('serialized_action') as lock:
        ...
        user = user_api.get_user_object(username)
        ...
        logging.info("adapter_info: %s", adapter_info)
        logging.info("hosts_info: %s", hosts_info)
...
class ActionHelper(object):
    ...
    def get_adapter_info(adapter_id, cluster_id, user):
        ...
        """
        adapter_info = adapter_api.get_adapter(user, adapter_id)
        metadata = cluster_api.get_cluster_metadata(user, cluster_id)
    ...
    def get_cluster_info(cluster_id, user):
        ...
        """
        cluster_info = cluster_api.get_cluster(user, cluster_id)
        cluster_config = cluster_api.get_cluster_config(user, cluster_id)
        cluster_info.update(cluster_config)

        deploy_config = cluster_api.get_cluster_deployed_config(user,
                                                                cluster_id)
    ...
    def get_hosts_info(cluster_id, hosts_id_list, user):
        ...
        for clusterhost_id in hosts_id_list:
            info = cluster_api.get_clusterhost(user, clusterhost_id)
            host_id = info[const.HOST_ID]
            temp = host_api.get_host(user, host_id)
            config = cluster_api.get_cluster_host_config(user, cluster_id,
                                                         host_id)
            # Delete 'id' from temp
            del temp['id']
            info.update(temp)
    ...
    @staticmethod
    def save_deployed_config(deployed_config, user=None):
        cluster_config = deployed_config[const.CLUSTER]
        cluster_id = cluster_config[const.ID]
        del cluster_config[const.ID]
        # TODO(Grace Yu): db_api set cluster deploy config
        cluster_api.update_cluster_deployed_config(user, cluster_id,
                                                   **cluster_config)
        ...
        for clusterhost_id in hosts_id_list:
            ...
            cluster_api.update_clusterhost_deployed_config(user,
                                                           clusterhost_id,
                                                           **config)
