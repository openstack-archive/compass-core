...
class MiscCheck(base.BaseCheck):
    ...
    MISC_MAPPING = {
        ...
        "enable"  : "httpd squid xinetd dhcpd named sshd rsyslog cobblerd ntpd compassd".split(" "),
    }
    ...
    def run(self):
        ...
        if self.code == 1:
            self.messages.append("[%s]Info: Miscellaneous check has completed. No problems found, all systems go." % self.NAME)
    ...
    def check_linux_dependencies(self):
        ...
        try:
            ...
            self._set_status(0, "[%s]Error: No module named %s, please install it first." % (self.NAME, pkg_module))
    ...
    def check_yum_dependencies(self, pkg_module):
        ...
        for package in self.MISC_MAPPING["yum"]:
            if not yum_base.rpmdb.searchNevra(name=package):
                self._set_status(0, "[%s]Error: %s package is required" % (self.NAME, package))
                uninstalled.append(package)

        if len(uninstalled) != 0:
            self._set_status(0, "[%s]Info: Uninstalled yum packages: %s" % (self.NAME, ', '.join(item for item in uninstalled)))
    ...
    def check_pip_dependencies(self):
        ...
        for module in self.MISC_MAPPING['pip']:
            try:
                ...
                self._set_status(0, "[%s]Error: pip package %s is requred" % (self.NAME, module))
                uninstalled.append(module)

        if len(uninstalled) != 0:
            self._set_status(0, "[%s]Info: Uninstalled pip packages: %s" % (self.NAME, ', '.join(item for item in uninstalled)))
    ...
    def check_chkconfig(self):
        ...
        for serv in self.MISC_MAPPING["disable"]:
            if health_check_utils.check_chkconfig(serv) == True:
                self._set_status(0, "[Miscellaenous Check]Error: %s is not disabled" % serv)
                serv_to_disable.append(serv)
        if len(serv_to_disable) != 0:
            self._set_status(0, "[Miscellaenous Check]Info: You need to disable these services on system start-up: %s" % ", ".join(item for item in serv_to_disable))

        serv_to_enable = []
        for serv in self.MISC_MAPPING["enable"]:
            if health_check_utils.check_chkconfig(serv) == False:
                self._set_status(0, "[Miscellaenous Check]Error: %s is disabled" % serv)
                serv_to_enable.append(serv)
        if len(serv_to_enable) != 0:
            self._set_status(0, "[Miscellaenous Check]Info: You need to enable these services on system start-up: %s" % ", ".join(item for item in serv_to_enable))
    ...
    def check_selinux(self):
        ...
        for line in f.readlines():
            if "SELINUX=disabled" in line:
                ...
                break
        if disabled == False:
            self._set_status(0, "[%s]Selinux is not disabled, please disable it in /etc/selinux/config." % self.NAME)
