...
def add_adapters_internal(force=False):
    global OSES
    if force or OSES is None:
        OSES = _get_oses()
    global OS_INSTALLERS
    if force or OS_INSTALLERS is None:
        OS_INSTALLERS = _get_os_installers()
    global PACKAGE_INSTALLERS
    if force or PACKAGE_INSTALLERS is None:
        PACKAGE_INSTALLERS = _get_package_installers()
    global ADAPTERS
    if force or ADAPTERS is None:
        ADAPTERS = _get_adapters()
    global ADAPTERS_ROLES
    if force or ADAPTERS_ROLES is None:
        ADAPTERS_ROLES = {}
        _add_roles()
    global ADAPTERS_FLAVORS
    if force or ADAPTERS_FLAVORS is None:
        ADAPTERS_FLAVORS = {}
        _add_flavors()
