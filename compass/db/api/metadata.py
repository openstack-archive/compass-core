...
def add_os_field_internal(session):
    configs = util.load_configs(setting.OS_FIELD_DIR, env_locals=validator.VALIDATOR_LOCALS)
    with session.begin(subtransactions=True):
        return _add_field_internal(session, models.OSConfigField, configs)


def add_package_field_internal(session):
    configs = util.load_configs(setting.PACKAGE_FIELD_DIR, env_locals=validator.VALIDATOR_LOCALS)
    with session.begin(subtransactions=True):
...
def _validate_self(config_path, config_key, config, metadata, whole_check):
    field_type = metadata['_self']['field_type']
    if not isinstance(config, field_type):
        raise exception.InvalidParameter(
            '%s config type is not %s' % (config_path, field_type)
        )
    required_in_options = metadata['_self']['required_in_options']
    options = metadata['_self']['options']
    ...
    validator = metadata['_self']['validator']
    if validator:
...
def _validate_config(config_path, config, metadata, whole_check):
    ...
    for key in redundant_keys:
        if specified[key]['_self']['is_required']:
            raise exception.InvalidParameter(
                '%s key %s not found in config' % (
                    config_path, key
                )
            )
        if (
            whole_check and
            specified[key]['_self']['required_in_whole_config']
        ):
            raise exception.InvalidParameter(
                '%s key %s not found in config' % (
                    config_path, key
