...
class CobblerInstaller(OSInstaller):
    ...
    def __init__(self, adapter_info, cluster_info, hosts_info):
        ...
        logging.info("cobbler==> adapter_info: %s", adapter_info)
        logging.info("cluster_info===> cluster_info : %s", cluster_info)
        logging.info("hosts_info====> hosts_info: %s", hosts_info)

        self.config_manager = CobblerConfigManager(adapter_info,
                                                   cluster_info,
                                                   hosts_info)
        installer_settings = self.config_manager.get_os_installer_settings()
        logging.info("cobbler===> installer_settings: %s", installer_settings)
        try:
    ...
    def deploy(self):
        ...
        self._sync()
        logging.info("cluster_deconfig: %s", cluster_deconfig)
        logging.info("hosts_deploy_config: %s", hosts_deploy_config)
        return {
            const.CLUSTER: {
    ...
    def _get_system_config(self, host_id, vars_dict):
        ...
        """
        logging.info("cobbler vars_dict: %s", vars_dict)
        os_version = self.config_manager.get_os_version()

        system_tmpl_path = os.path.join(os.path.join(self.tmpl_dir,
                                                     os_version),
                                        self.SYS_TMPL_NAME)
        logging.info("cobbler system template path: %s", system_tmpl_path)
        ...
        logging.info("cobbler system config-====> %s", system_config)
    ...
    def _get_profile_from_server(self, os_version):
        ...
        profile = result[0]
        logging.info("cobbler profile===========> %s", profile)
        return profile
