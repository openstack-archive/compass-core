...
def export_database(session, table=None):
    ...
    if table is not None:
        ...
        remove_column = ['created_at', 'updated_at', 'id']
        if table.__tablename__ is not 'host':
            header = [[column.name for column in table.__mapper__.columns if column.name not in remove_column]]
        else:
            header = [[column.name for column in table.__mapper__.columns if column.name not in ['created_at', 'updated_at']]]
        ...
        for table in dir(models):
            ...
            if hasattr(table_object, 'able_to_export') and table_object.able_to_export == True:
...
def import_database(session, import_data):
    ...
    foreign_tables = []
    foreign_objects = []
    second_tables = ['clusterhost']
    second_objects = [models.ClusterHost]
    for table in dir(models):
        table_object = getattr(models, table)
        if hasattr(table_object, 'able_to_export') and table_object.able_to_export == True:
                    ...
                    for key, value in data.items():
                        if '.' in key:
                            ...
                            if foreign_data[0] == 'creator':
                                user = get_db_object(session, models.User, email=value)
                                data['creator_id'] = user.id
                                data.pop(key)
                                break
                            elif foreign_data[0] == 'switch':
                                switch = get_db_object(session, models.Switch, ip_int=value)
                                data['switch_id'] = switch.id
                                data.pop(key)
                                break
                            elif foreign_data[0] == 'machine':
                                machine = get_db_object(session, models.Machine, mac=value)
                                for k, v in foreign_keys.items():
                                    if v == key:
                                        data[k] = machine.id
                                        data.pop(key)
                            elif foreign_data[0] == 'subnet':
                                subnet = get_db_object(session, models.Subnet, subnet=value)
                                data['subnet_id'] = subnet.id
                                data.pop(key)
                                break
                            else:
                                model_name = ''.join(name.capitalize() for name in foreign_data[0].split('_'))
                                if model_name == 'Os':
                                ...
                                object = get_db_object(session, getattr(models, model_name), name=value)
                                for k, v in foreign_keys.items():
                                    if v == key:
                                        data[k] = object.id
                                        data.pop(key)
