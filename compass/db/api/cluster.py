...
UPDATED_FIELDS = ['name', 'adapter_id']
UPDATED_CLUSTERHOST_FIELDS = ['name']
UPDATED_HOST_FIELDS = ['name']
...
@utils.supported_filters(optional_support_keys=UPDATED_CLUSTERHOST_FIELDS)
def add_clusterhost_internal(session, cluster, machine_id, **kwargs):
    ...
    with session.begin(subtransactions=True):
        ...
        utils.add_db_object(
            session, models.ClusterHost, False,
...
def _add_clusterhosts(session, cluster, machines):
    with session.begin(subtransactions=True):
        for machine_id, host_attrs in machines.items():
            add_clusterhost_internal(
                session, cluster, machine_id, **host_attrs
            )
...
def get_clusterhost(getter, clusterhost_id, **kwargs):
    ...
    with database.session() as session:
        user_api.check_user_permission_internal(
            session, getter, permission.PERMISSION_LIST_CLUSTERHOSTS)
        return utils.get_db_object(
            session, models.Cluster, id=clusterhost_id
        ).to_dict()
...
@utils.wrap_to_dict(RESP_CLUSTERHOST_CONFIG_FIELDS)
@utils.supported_filters(UPDATED_CLUSTERHOST_CONFIG_FIELDS)
...
@utils.wrap_to_dict(RESP_CLUSTERHOST_STATE_FIELDS)
@utils.supported_filters(UPDATED_CLUSTERHOST_STATE_FIELDS)
