...
class MachineList(Resource):
    ...
    def get(self):
        ...
        with database.session() as session:
            ...
            filter_ports = []
            if switch_id:
                # Get all ports associated with the switch_id which is expected
                # to be filtered.
                filter_ports = session.query(SwitchConfig.filter_port)\
                    .join(ModelSwitch)\
                    .filter(SwitchConfig.ip == ModelSwitch.ip)\
                    .filter(ModelSwitch.id == switch_id).all()
            else:
                # Get all ports which are expected to be filtered.
                filter_ports = session.query(SwitchConfig.filter_port)\
                    .join(ModelSwitch)\
                    .filter(SwitchConfig.ip == ModelSwitch.ip).all()

            if filter_ports:
                filter_ports = [i[0] for i in filter_ports]
                if port in filter_ports:
                    filter_ports.remove(port)

            logging.debug("fiter posts are %s", filter_ports)
            for machine in machines:
                if machine.port in filter_ports:
                    continue
