...
def _get_host_network(session, host_network_id, **kwargs):
    ...
    elif not isinstance(host_network_id, basestring):
        raise exception.InvalidParameter(
            'host network %s type is not string' % host_network_id
        )
    elif re.match(r'^\d+$', host_network_id):
        return utils.get_db_object(
            session, models.HostNetwork,
            id=int(host_network_id), **kwargs
        )
    elif re.match('^\d+\.\d+\.\d+\.\d+$', host_network_id):
        ip_int = long(netaddr.IPAddress(host_network_id))
        return utils.get_db_object(
            session, models.HostNetwork,
            ip_int=ip_int, **kwargs
        )
    elif ':' in host_network_id:
        host_id, interface = host_network_id.split(':', 1)
        host = _get_host(session, host_id)
        return utils.get_db_object(
            session, models.HostNetwork,
            host_id=host.id, interface=interface, **kwargs
        )
    else:
        raise exception.InvalidParameter(
            'host_network_id %s format is not acceptable' % host_network_id
        )
...
def update_host_state_internal(
    ...
    """Update a host state."""
    host = _get_host(session, hostname)
    ...
    logging.debug('host %s ready: %s', hostname, ready_triggered)
