...
class SquidCheck(base.BaseCheck):
    ...
    def check_squid_files(self):
        ...
        if not conf_err_msg == "":
            ...
            self._set_status(0, "[%s]Error: squid.conf has incorrect file permissions" % self.NAME)
        else:
        ...
        if not squid_path_err_msg == "":
            ...
            self._set_status(0, health_check_utils.check_path(self.NAME, '/var/squid/cache'))
        else:
            ...
            if uid != gid or pwd.getpwuid(23).pw_name != 'squid':
                self._set_status(0, "[%s]Error: /var/squid directory ownership misconfigured" % self.NAME)
            else:
    ...
    def check_squid_service(self):
        ...
        if not 'squid' in commands.getoutput('ps -ef'):
            self._set_status(0, "[%s]Error: squid service does not seem running" % self.NAME)

        try:
            if 'squid' != getservbyport(3128):
                self._set_status(0, "[%s]Error: squid is not listening on 3128" % self.NAME)
        except:
            self._set_status(0, "[%s]Error: No service is listening on 3128, squid failed" % self.NAME)
        return True
