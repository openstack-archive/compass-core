...
class CeleryCheck(base.BaseCheck):
    ...
    def check_compass_celery_setting(self):
        ...
        if len(unset) != 0:
            self._set_status(0, "[%s]Error: Unset celery settings: %s in /etc/compass/setting" % (self.NAME, ', '.join(item for item in unset)))
        return True
    ...
    def check_celery_backend(self):
        ...
        if not 'celeryd' in commands.getoutput('ps -ef'):
            self._set_status(0, "[%s]Error: celeryd is not running" % self.NAME)
            return True

        if not os.path.exists('/etc/compass/celeryconfig'):
            self._set_status(0, "[%s]Error: No celery config file found for Compass" % self.NAME)
            return True
        ...
        try:
            ...
            if not celery_stats:
                self._set_status(0, "[%s]Error: No running Celery workers were found." % self.NAME)
        except IOError as e:
            self._set_status(0, "[%s]Error: Failed to connect to the backend: %s" % (self.NAME, str(e)))
