...
class DhcpCheck(base.BaseCheck):
        ...
        if os.path.exists("/etc/cobbler/dhcp.template"):
            ...
            for var in VAR_MAP.keys():
                if VAR_MAP[var] == False:
                    failed.append(var)
            if len(failed) != 0:
                self._set_status(0, "[%s]Info: DHCP template file failed components: %s" % (self.NAME, ' '.join(f for f in failed)))
        else:
            self._set_status(0, "[%s]Error: DHCP template file doesn't exist, health check failed." % self.NAME)
        return True
    ...
    def check_dhcp_service(self):
       ...
       if not 'dhcp' in commands.getoutput('ps -ef'):
            self._set_status(0, "[%s]Error: dhcp service does not seem to be running" % self.NAME)
       if getservbyport(67) != 'bootps':
           self._set_status(0, "[%s]Error: bootps is not listening on port 67" % self.NAME)
       return True
