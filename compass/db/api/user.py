...
UPDATED_FIELDS = ['firstname', 'lastname', 'password', 'is_admin', 'active']
...
def _check_user_permission(session, user, permission_name):
    ...
    with session.begin(subtransactions=True):
        ...
        user_permission = utils.get_db_object(
            ...
            False, user_id=user.id, name=permission_name
        )
...
def check_user_permission_internal(session, user, permission_name):
...
def _add_user_permissions(session, user, **permission_filters):
    ...
    with session.begin(subtransactions=True):
        existing_permissions = dict([
            (user_permission.permission_id, user_permission)
        ...
        for permission in permission_api.list_permissions_internal(
            ...
            existing_permissions[permission.id] = (
                models.UserPermission(user.id, permission.id)
            )

        user.user_permissions = existing_permissions.values()
...
def _remove_user_permissions(session, user, **permission_filters):
    ...
    with session.begin(subtransactions=True):
        ...
        for permission in permission_api.list_permissions_internal(
            ...
            del existing_permissions[permission.id]
...
def _set_user_permissions(session, user, **permission_filters):
    ...
    with session.begin(subtransactions=True):
        ...
        remove_permissions = existing_permissions.keys()
        for permission in permission_api.list_permissions_internal(
            session, **permission_filters
        ):
            if permission.id not in existing_permissions:
                ...
                remove_permissions.remove(permission.id)
...
def add_permission(creator, user_id, name):
    ...
    with database.session() as session:
        ...
        user_permission = utils.add_db_object(
            session, models.UserPermission, True, user_id, permission.id
        )
        return user_permission.to_dict() 
