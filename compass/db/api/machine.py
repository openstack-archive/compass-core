...
def _get_machine(session, machine_id, **kwargs):
    if isinstance(machine_id, (int, long)):
        ...
        raise exception.InvalidParameter(
            'machine %s type is not string' % machine_id
        )
    elif re.match(r'^\d+$', machine_id):
        return utils.get_db_object(
            session, models.Machine,
            id=int(machine_id), **kwargs
        )
    else:
        return utils.get_db_object(
            session, models.Machine,
            mac=machine_id, **kwargs
        )
