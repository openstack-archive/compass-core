...
class InstallerMixin(HelperMixin):
    ...
    def initialize(self):
        ...
        super(InstallerMixin, self).initialize()
...
class ClusterHost(BASE, TimestampMixin, HelperMixin):
    ...
    def initialize(self):
        self.state.host_state = self.host.state
        self.state.cluster_state = self.cluster.state

    @property
    def name(self):
        return '%s.%s' % (self.host.name, self.clsuter.name)
    ...
    def put_package_config(self, value):
        self.package_config.update(value)
    ...
    def state_dict(self):
        if self.state.progress <= 0.0:
            if not self.cluster.distributed_system:
                dict_info = self.host.state_dict()
            else:
                dict_info = self.host.state_dict()
                dict_info['state'] = self.state.state
        else:
            dict_info = self.state.to_dict()
        return dict_info
...
class Host(BASE, TimestampMixin, HelperMixin):
    ...
    def put_os_config(self, value):
        os_config = self.os_config
        self.os_config.update(value)

    def __init__(self, machine_id, **kwargs):
        self.id = machine_id
...
class ClusterState(BASE, StateMixin):
    ...
    def initialize(self):
        if self.state == 'INSTALLING':
            self.cluster.reinstall_distributed_system = False
...
class Cluster(BASE, TimestampMixin, HelperMixin):
    ...
    def intialize(self):
        ...
        if package_adapter:
            self.distributed_system = package_adapter.adapter_distributed_system
        else:
    ...
    def distributed_system_name(self):
        if self.distributed_system:
            return self.distributed_system.name
        else:
            return None
...
class UserToken(BASE, HelperMixin):
    ...
    def initialize(self):
        ...
        super(UserToken, self).initialize()
...
class Machine(BASE, HelperMixin, TimestampMixin):
    ...
    def patched_ipmi_credentials(self, value):
        ipmi_credentials = self.ipmi_credentials
    ...
    def patched_tag(self, value):
        tag = self.tag
        self.tag = util.merge_dict(tag, value) 
    ...
    def patched_location(self, value):
        ...
        self.location = util.merge_dict(location, value)
...
class Switch(BASE, HelperMixin, TimestampMixin):
    ...
    credentials = Column(MutableDict.as_mutable(JSONEncoded), default={})
...
class Adapter(BASE, HelperMixin):
    ...
    package_adapter_id = Column(
        Integer,
        ForeignKey('package_adapter.id', onupdate='CASCADE', ondelete='CASCADE'),
        nullable=True
    )
    ...
    __table_args__ = (
        UniqueConstraint('package_adapter_id', 'os_adapter_id', name='constraint'),
    )
...
class OSConfigMetadata(BASE, MetadataMixin):
    ...
    parent_id = Column(
        Integer, ForeignKey('os_config_metadata.id', onupdate='CASCADE', ondelete='CASCADE')
    )
    field_id = Column(
        Integer, ForeignKey('os_config_field.id', onupdate='CASCADE', ondelete='CASCADE')
    )
...
class PackageConfigMetadata(BASE, MetadataMixin):
    ...
    parent_id = Column(
        Integer, ForeignKey('package_config_metadata.id', onupdate='CASCADE', ondelete='CASCADE')
    )
    field_id = Column(
        Integer, ForeignKey('package_config_field.id', onupdate='CASCADE', ondelete='CASCADE')
    )
    children = relationship(
    ...
    def initialize(self):
        super(PackageConfigMetadata, self).initialize()
...
class PackageAdapter(BASE, AdapterMixin):
    ...
    distributed_system_id = Column(
        Integer,
        ForeignKey('distributed_system.id', onupdate='CASCADE', ondelete='CASCADE'),
        nullable=True
    )
    installer_id = Column(
        Integer,
        ForeignKey('package_installer.id', onupdate='CASCADE', ondelete='CASCADE'),
        nullable=True
    )
    support_os_patterns = Column(JSONEncoded, default=[])
    ...
    __table_args__ = (
        UniqueConstraint('distributed_system_id', 'installer_id', name='constraint'),
    )
    ...
    @property
    def adapter_support_os_patterns(self):
        support_os_patterns = self.support_os_patterns
        if support_os_patterns:
            return support_os_patterns
        parent = self.parent
        if parent:
            return parent.adapter_support_os_patterns
        else:
            return []
    ...
    def to_dict(self):
        ...
        dict_info['supported_os_pattern'] = self.adapter_supported_os_pattern
