...
class OsInstallerCheck(base.BaseCheck):
    ...
    def cobbler_check(self):
        ...
        if len(self.remote.get_distros()) == 0:
            self._set_status(0, "[%s]Error: No Cobbler distros found" % self.NAME)

        if len(self.remote.get_profiles()) == 0:
            self._set_status(0, "[%s]Error: No Cobbler profiles found" % self.NAME)
        ...
        if len(self.remote.get_repos()) != 0:
            for repo in self.remote.get_repos():
                if 'ppa_repo' in repo['mirror']:
                    ...
                    break
        if found_ppa == False:
            self._set_status(0, "[%s]Error: No repository ppa_repo found" % self.NAME)
        ...
        if len(not_exists) != 0:
            self._set_status(0, "[%s]Error: These locations do not exist: %s" % (self.NAME, ', '.join(item for item in not_exists)))
