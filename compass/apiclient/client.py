...
def _poll_switches(client):
    ...
    for switch_ip in switch_ips:
        if switch_ip not in all_switches:
            ...
            logging.info('add a switch status: %s resp: %s', status, resp)
            if status >= 400:
    ...
    for switch_ip, switch in all_switches.items():
        ...
        # the switch state.
        remain_retries = flags.OPTIONS.max_retries
...
def _get_machines(client):
    ...
    logging.info('machine to add: %s', machines.values())
...
def _set_cluster_security(client, cluster_hosts):
    ...
    for credential in credentials:
        ...
        if credential_name and ('=' in username_and_password):
            username, password = username_and_password.split('=', 1)
            if username and password:
                credential_mapping['%s_username'] = username
                credential_mapping['%s_password'] = password
...
def _set_cluster_networking(client, cluster_hosts):
    ...
    networking = {}
    for networking in flags.OPTIONS.networking.split(';'):
        if not networking:
            continue
        ...
        if '=' in networking:
            ...
            if networking_name.endswith('_promisc'):
                networking[networking_name] = int(networking_value)
            else:
                networking[networking_name] = networking_value
    ...
    for cluster_id, host_ids in cluster_hosts.items():
        ...
        logging.info(
            ...
            networking, cluster_id, status, resp)
        if status >= 400:
            msg = 'failed to set networking config %s to cluster %s' % (
                networking, cluster_id)
            raise Exception(msg)
...
def _set_cluster_partition(client, cluster_hosts):
    ...
    for partition in partitions:
        ...
        if partition_pair and '=' in partition_pair:
