...
if [ $tempest-full == true ]; then
    ...
    nosetests -sv --attr=type=smoke tempest.tests.compute.images.test_list_images:ListImagesTest.test_list_images_with_detail
    nosetests -sv --attr=type=smoke tempest.tests.compute.servers.test_create_server:ServersTestAutoDisk.test_verify_server_details
    nosetests -sv --attr=type=smoke tempest.tests.compute.servers.test_server_addresses:ServerAddressesTest.test_list_server_addresses
    nosetests -sv --attr=type=smoke tempest.tests.compute.volumes.test_volumes_get:VolumesGetTestJSON.test_volume_create_get_delete
    nosetests -sv --attr=type=smoke tempest.tests.identity.admin.test_users:UsersTestJSON.test_get_users
    nosetests -sv --attr=type=smoke tempest.tests.volume.test_volumes_get:VolumesGetTest.test_volume_create_get_delete
fi
