...
def get_col_val_from_dict(values, data):
    """
    Convert a dict to cols with name and value.
       nested keys in dict will be joined by '.' as a column name in CSV.
       :param row: a list of values for each column
       :param data: input data

       return a list of column names and its value
       for example:
       data = {"a": {"b": {"c": 1},
                     "d": 2}}
       return [1, 2]
    ...
    for key in data:
        get_col_val_from_dict(values, data[key])


def get_headers_from_dict(headers, dname, data):
    if not isinstance(data, dict):
        headers.append(dname)
        return

    for key in data:
        tmp_header = '.'.join((dname, key))
