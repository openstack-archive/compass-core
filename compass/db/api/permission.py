...
def _get_permission(session, permission_id, **kwargs):
    ...
    elif not isinstance(permission_id, basestring):
        raise exception.InvalidParameter(
            'permission %s type is not string' % permission_id
        )
    elif re.match('^\d+$', permission_id):
        return utils.get_db_object(
            session, models.Permission, id=int(permission_id), **kwargs)
    else:
        return utils.get_db_object(
            session, models.Permission, name=permission_id, **kwargs)
