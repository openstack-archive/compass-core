...
from comapss.db.api import cluster as cluster_api
from compass.db.api import clusterhost as clusterhost_api
...
def _filter_location(data):
    location_filter = {}
    locatins = _get_data_list(data, 'location')
    if locations:
...
def add_cluster():
    ...
    return utils.make_json_response(
        200,
        clsuter_api.add_cluster(current_user, **data)
    )
...
@app.route("/clusters/<int:cluster_id>/clusterhosts", methods=['GET'])
@log_user_action
@login_required
...
def take_cluster_action(cluster_id):
    ...
    if 'deploy' in data:
        return utils.make_json_response(
            202,
            cluster_api.deploy_cluster(current_user, cluster_id, **data['deploy'])
        )
    return utils.make_json_response(
        200,
        {
...
def show_clusterhost(clusterhost_id):
    ...
    return utils.make_json_response(
        200,
        clusterhost_api.get_clusterhost(
...
def show_clusterhost_config(clusterhost_id):
    ...
    return utils.make_json_response(
        200,
        clusterhost_api.get_clusterhost_config(
            current_user, clusterhost_id, **data
        )
    )
...
def update_clusterhost_config(clusterhost_id):
    ...
    return utils.make_json_response(
        200,
        clusterhost_api.update_clusterhost_config(current_user, clusterhost_id, **data)
    )
...
def patch_clusterhost_config(clusterhost_id):
    ...
    return utils.make_json_response(
        200,
        clusterhost_api.patch_clusterhost_config(current_user, clusterhost_id, **data)
    )
...
def delete_clusterhost_config(clusterhost_id):
    ...
    return utils.make_json_response(
        200,
        clusterhost_api.del_clusterhost_config(
            current_user, clusterhost_id, **data
        )
    )
...
def show_clusterhost_state(clusterhost_id):
    ...
    return utils.make_json_response(
        200,
        clusterhost_api.get_clusterhost_state(
            current_user, clusterhost_id, **data
        )
    )
...
def update_clusterhost_state(clusterhost_id):
    ...
    return utils.make_json_response(
        200,
        clusterhost_api.update_clusterhost_state(
            current_user, clusterhost_id, **data
        )
    )
