...
SUPPORTED_FILTERS = ['name', 'os_name', 'owner', 'mac']
RESP_FIELDS = [
...
UPDATED_NETWORK_FIELDS=['interface', 'ip', 'subnet_id', 'is_mgmt', 'is_promiscuous']
RESP_STATE_FIELDS = [
    ...
    with session.begin(subtransactions=True):
        if reinstall_os_set:
            if host.state.state == 'INSTALLING':
                return _conditional_exception(host, exception_when_not_editable)
...
def add_host_network(creator, host_id, **kwargs):
    ...
    with database.session() as session:
        ...
        check_host_editable(session, host, creator)
...
def del_host_network(deleter, host_network_id, **kwargs):
    ...
    with database.session() as session:
        ...
        check_host_editable(session, host_network.host, deleter)
