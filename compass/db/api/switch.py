...
def _get_switch(session, switch_id, **kwargs):
    ...
    elif not isinstance(switch_id, basestring):
        raise exception.InvalidParameter(
            'switch %s type is not string' % switch_id
        )
    elif re.match(r'^\d+$', switch_id):
        return utils.get_db_object(
            session, models.Switch,
            id=int(switch_id), **kwargs
        )
    else:
        ip_int = long(netaddr.IPAddress(switch_id))
        return utils.get_db_object(
            session, models.Switch,
