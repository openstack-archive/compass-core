...
def _range_condition(col_attr, value):
    conditions = []
    if value[0] is not None:
        conditions.append(col_attr >= value[0])
    if value[1] is not None:
        conditions.append(col_attr <= value[1])
    if not conditions:
        return None
    else:
...
def model_filter(query, model, **filters):
    ...
    for key, value in filters.items():
        ...
        if isinstance(value, list):
            ...
            if 'in' in value:
                query = query.filter(col_atttr.in_(value['in']))
...
def add_db_object(session, table, exception_when_existing=True,
    ...
    with session.begin(subtransactions=True):
        ...
        arg_names = inspect.getargspec(table.__init__).args[1:]
...
IPADDR_PATTERN = re.compile(r'(\d+)\.(\d+)\.(\d+)\.(\d+)')
IPADDR_FIELD_OFFSETS = [1,2,3,4]


def check_ip(ip):
    mat = IPADDR_PATTERN.match(ip)
    if not mat:
        raise exception.InvalidParameter(
            'invalid ip address %s' % ip
        )

    for i in IPADDR_FIELD_OFFSETS:
        ip_field = int(mat.group(i))
        if ip_field > 255:
            raise exception.InvalidParameter(
                 'field %d in %s should be in range [0,255] but it is %s' % (
                     i, ip, ip_field
                 )
            )
