...
for i in `seq $VIRT_NUM`; do
    ...
    echo "make pxe${i} reboot if failing."
    ...
    if [[ "$state" == "running" ]]; then
        echo "$virt_machine is already running"
        virsh destroy pxe${i}
        if [[ "$?" != "0" ]]; then
...
cp -rf ${REGTEST_DIR}/../compass/apiclient/client.py ${CLIENT_SCRIPT}
chmod +x ${CLIENT_SCRIPT}

/opt/compass/bin/refresh.sh

/usr/bin/python ${CLIENT_SCRIPT} --logfile= --loglevel=info --logdir= --networking="${NETWORKING}" --partitions="${PARTITION}" --credentials="${SECURITY}" --host_roles="${host_roles_str}" --dashboard_role="${DASHBOARD_ROLE}" --switch_ips="${SWITCH_IPS}" --machines="${machines_str}" --switch_credential="${SWITCH_CREDENTIAL}"
if [[ "$?" != "0" ]]; then
